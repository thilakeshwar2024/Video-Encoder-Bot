...
async def encode(filepath, message, msg):
    ...
    if m:
        metadata = '-metadata title=ToonEncodes.one -metadata:s:v title=ToonEncodes.one -metadata:s:a title=ToonEncodes.one -metadata:s:s title=Encoded By ToonEncodes.one'
    else:
        metadata = ''
